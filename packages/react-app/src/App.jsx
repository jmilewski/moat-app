...
import { Hints, ExampleUI, Subgraph, DeFiPulse, CustomTokenView } from "./views"
import { useThemeSwitcher } from "react-css-theme-switcher";
import { INFURA_ID, DAI_ADDRESS, DAI_ABI, NETWORK, NETWORKS, DEFIPULSE_ADDRESS, DEFIPULSE_ABI } from "./constants";
/*
...
function App(props) {
  ...
  const mainnetDeFiPulseContract = useExternalContractLoader(mainnetProvider, DEFIPULSE_ADDRESS, DEFIPULSE_ABI)
  const componentsDeFiPulse = useContractReader({"DefiPulse Index": mainnetDeFiPulseContract}, "DefiPulse Index", "getComponents")
  ...
  */
  ...
  useEffect(()=>{
    if(DEBUG && mainnetProvider && address && selectedChainId && yourLocalBalance && yourMainnetBalance && readContracts && writeContracts && mainnetDAIContract){
      ...
      console.log("DeFi Pulse contract on mainnet",mainnetDeFiPulseContract)
  ...
  useEffect(() => {
    const fetchChartAndTableIndexPrices = async () => {
      let qs = '?symbol=UNI,MKR,AAVE,SNX,COMP&interval=weekly'
      const result = await axios(
        'https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/historical' + qs, {
          headers: { "X-CMC_PRO_API_KEY": 'e14f8f11-49bd-47b1-9c14-39c303b2a68a' }
      ...
      var tokenQuotes = [quotesUNI, quotesMKR, quotesAAVE, quotesSNX, quotesCOMP];

      var timestampList = [];
      quotesMKR.forEach(item => {
        timestampList.push(item.timestamp);
      });

      var dataTimestampMarketCaps = {
        0: [],
        1: [],
        2: [],
        3: [],
      ...
      var marketCapSumList = [];

      const createTokenMarketCaps = () => {
        tokenQuotes.forEach(item => {
          var step = 0;
          item.forEach(quoteItem => {
            dataTimestampMarketCaps[step].push(quoteItem.quote['USD'].market_cap);
            step++
          });
        });
      }
      createTokenMarketCaps();
      
      const reducer = (accumulator, currentValue) => accumulator + currentValue;
      var counter = 0;
      for (const property in dataTimestampMarketCaps) {
         marketCapSumList.push(dataTimestampMarketCaps[counter].reduce(reducer));
         counter++;
      }

      const indexConstant = 267892709;

      var indexPricesWeekly = [];

      marketCapSumList.forEach(item => {
        let indexCalc = item/indexConstant;
        indexPricesWeekly.push(indexCalc);
      });

      var indexPricesWeeklyTableData = [];

      indexPricesWeekly.forEach(item => {
        indexPricesWeeklyTableData.push({ date: "Today", price: item })
      });

      var counter2 = 0;
      timestampList.forEach(item => {
        indexPricesWeeklyTableData[counter2].date = item;
        counter2++;
      })

      setIndexPrices(indexPricesWeeklyTableData);

    }
    fetchChartAndTableIndexPrices();
    ...
    <div className="App">
      ...
      {/* 👨‍💼 Your account is in the top right with a wallet at connect options */}
        ...
        <Menu style={{ textAlign:"left" }} selectedKeys={[route]} mode="horizontal">
          ...
          <Menu.Item key="/defipulse">
            <Link onClick={()=>{setRoute("/defipulse")}} to="/defipulse">Blue Crypts</Link>
          </Menu.Item>
          <Menu.Item key="/customtoken">
            <Link onClick={()=>{setRoute("/customtoken")}} to="/customtoken">Custom Index</Link>
          </Menu.Item>
        </Menu>
        ...
        <Switch>
          ...
          <Route path="/defipulse">
            <DeFiPulse
            tx={tx}
            writeContracts={writeContracts}
            mainnetProvider={mainnetProvider}
            yourLocalBalance={yourLocalBalance}
            componentsDeFiPulse={componentsDeFiPulse}
            indexPrices={indexPrices}
            />
          </Route>
          <Route path="/customtoken">
            <CustomTokenView
            tx={tx}
            writeContracts={writeContracts}
            mainnetProvider={mainnetProvider}
      ...
      {/* 👨‍💼 Your account is in the top right with a wallet at connect options */}
      {/* <div style={{ position: "relative", textAlign: "right", right: 0, top: 0, padding: 10 }}>
         <Account
           address={address}
           localProvider={localProvider}
           userProvider={userProvider}
